...
const allowed: Allowed = {
  ...
  "i18n/frontend-lib/src/geml-evaluation.tsx": {
    "@typescript-eslint/no-unnecessary-condition": 1,
    "@typescript-eslint/no-useless-template-literals": 1,
  },
  ...
  "i18n/frontend-lib/src/utils.ts": { "@typescript-eslint/no-explicit-any": 1 },
  ...
  "i18n/i18n-api/src/I18nConnection.ts": { "no-restricted-properties": 1 },
...
async function main() {
  ...
  for (const file of await cli.lintFiles(list)) {
    const filepath = path.relative(path.join(__dirname, ".."), file.filePath);
