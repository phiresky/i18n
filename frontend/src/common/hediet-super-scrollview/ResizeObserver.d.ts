 ...
 * [SVGElement](https://developer.mozilla.org/en-US/docs/Web/API/SVGElement).> **Note**: The content box is the box in which content can be placed, meaning
 * > the border box minus the padding and border width. The border box encompasses
 * > the content, padding, and border. See [The box
...
declare class ResizeObserver {
	 ...
	 * 					Math.max(1, entry.contentBoxSize.inlineSize / 600) +
	 * 					"rem";
	 * 			} else {
	 * 				h1Elem.style.fontSize =
	 ...
	 *       objects that can be used to access the new dimensions of the
	 *       element after each change.
	 *   - **observer** A reference to the `ResizeObserver` itself, so it will
	 *       definitely be accessible from inside the callback, should you need
	 *       it. This could be used for example to automatically unobserve the
	 *       observer when a certain condition is reached, but you can omit it
	 ...
	 *   ([see
	 *   source](https://github.com/mdn/dom-examples/blob/master/resize-observer/resize-observer-text.html))
	 *   example:
	 */
	 ...
	 *
	 * @param target A reference to an
	 *   [Element](https://developer.mozilla.org/en-US/docs/Web/API/Element) or
	 *   [SVGElement](https://developer.mozilla.org/en-US/docs/Web/API/SVGElement)
	 *   to be observed.
	 * @param options An options object allowing you to set options for the
	 *   observation. Currently this only has one possible option that can be
	 *   set.
	 */
...
interface ResizeObserverObserveOptions {
	/**
	 * Sets which box model the observer will observe changes to. Possible
	 * values are `content-box` (the default), and `border-box`.
	 *
	 * @default "content-box"
	 */
 ...
 * 			if (entry.contentBoxSize) {
 * 				h1Elem.style.fontSize =
 * 					Math.max(1.5, entry.contentBoxSize.inlineSize / 200) +
 * 					"rem";
 * 				pElem.style.fontSize =
 * 					Math.max(1, entry.contentBoxSize.inlineSize / 600) +
 * 					"rem";
 ...
 * 	resizeObserver.observe(divElem);
 *
 * @param entries An array of
 *   [ResizeObserverEntry](https://developer.mozilla.org/en-US/docs/Web/API/ResizeObserverEntry)
 *   objects that can be used to access the new dimensions of the element after
 *   each change.
 * @param observer A reference to the `ResizeObserver` itself, so it will
 *   definitely be accessible from inside the callback, should you need it. This
 *   could be used for example to automatically unobserve the observer when a
 *   certain condition is reached, but you can omit it if you don't need it.
 *
 *   The callback will generally follow a pattern along the lines of:
 */
...
interface ResizeObserverEntry {
	 ...
	 * is run. Note that this is better supported than the above two properties,
	 * but it is left over from an earlier implementation of the Resize Observer
	 * API, is still included in the spec for web compat reasons, and may be
...
interface ResizeObserverEntryBoxSize {
	 ...
	 * this is the vertical dimension, or height; if the writing-mode is
